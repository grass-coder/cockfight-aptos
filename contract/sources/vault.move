module cockfight::vault {
    ...
    public fun register_vault<VaultT, AssetT>(_host: &signer) {
        ...
        assert!(host_addr == @cockfight, E_NOT_AUTHORIZED);
    ...
    public (friend) fun deposit_vault<VaultT, AssetT>(_coin: Coin<AssetT>) acquires Vault {
        let vault = borrow_global_mut<Vault<VaultT, AssetT>>(@cockfight);
    ...
    fun call_test_setting(host: &signer, aptos_framework: &signer) {
        ...
        if (!account::exists_at(address_of(host))) {
            account::create_account_for_test(address_of(host));
        };
    ...
    #[test(host = @cockfight, aptos_framework = @aptos_framework)]
    /// Test initialize
